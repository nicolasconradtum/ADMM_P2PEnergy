...
from src.central_optimization.CentralOptimizer import CentralOptimizer
...
net3 = create_kerber_landnetz_freileitung_2()

agents1 = [
    Consumer(agent_id=1, p_min=-35, p_max=-30, utility_a=0.9, utility_b=13, power_factor=1),
    Consumer(agent_id=2, p_min=-20, p_max=-15, utility_a=1.25, utility_b=9.1, power_factor=1),
    Consumer(agent_id=3, p_min=-20, p_max=-12, utility_a=1, utility_b=16, power_factor=1),
    Consumer(agent_id=4, p_min=-25, p_max=-20, utility_a=1.1, utility_b=12, power_factor=1),
    Producer(agent_id=5, p_min=2, p_max=30, cost_a=0.26,cost_b=6, power_factor=1),
    Producer(agent_id=6, p_min=2, p_max=40, cost_a=0.4, cost_b=2.6, power_factor=1),
    Producer(agent_id=7, p_min=5, p_max=30, cost_a=0.6, cost_b=4, power_factor=1),
    Producer(agent_id=8, p_min=0, p_max=20, cost_a=0.7, cost_b=6.5, power_factor=1)
]
...
gm1 = GridManager(net1, agents1)
gm2 = GridManager(net2, agents2)
gm3 = GridManager(net3, agents3)
...
log_unconstrained = ADMM_Solver1.solve(inc=2, dec=2, delta=10)
log_constrained = ADMM_Solver2.solve(inc=2, dec=2, delta=10)

print("====== # ITERATIONS ======")
print("F-ADMM Unconstrained: " + str(log_unconstrained['iterations']))
print("F-ADMM Constrained: " + str(log_constrained['iterations']) +  "\n")

matprint(log_constrained["lambda"])
...
print("F-ADMM Unconstrained: ")
print("---")
v1, P_flow1, Q_flow1 = gm1.compute_lindistflow(log_unconstrained['P'])
...
central_optimizer = CentralOptimizer(gm3, agents3, True)
central_optimizer.solve()

print("Central Optimizer")
matprint(np.sum(central_optimizer.P.value,axis=1))
v3, P_flow3, Q_flow3 = gm3.compute_lindistflow(central_optimizer.P.value)
print("---")
print("F-ADMM Constrained")
matprint(np.sum(log_constrained['P'],axis=1))
print("---")
matprint(log_constrained['lambda'])
